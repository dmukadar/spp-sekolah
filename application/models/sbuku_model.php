<?php
class Sbuku_model extends Alazka_Controller { // Buat class 
...
*/ 	
	...
	$closed=$this->get_status("closed");
		$this->db->select("sis_siswa.id AS id_siswa, sis_siswa.namalengkap,dm_kelas.kelas, MONTHNAME( ar_payment_details.created ) AS bulan,ar_payment_details.id AS detail_id,ar_payment_details.amount, ar_payment_details.created,  ar_payment.notes,ar_invoice.description,ar_rate.name,ar_payment_details.modified, ar_payment_details.installment, sis_siswa.dm_jenjang_id,ar_payment_details.status,sum( case when ar_invoice.id_rate=30 or ar_invoice.id_rate=34 then (ar_payment_details.amount )  else 0 end) as uang_paket,sum( case when ar_invoice.id_rate=31 or ar_invoice.id_rate=35 then(ar_payment_details.amount) else 0 end) as uang_nonpaket,sum( case when ar_invoice.id_rate=30 or ar_invoice.id_rate=34 or ar_invoice.id_rate=31 or ar_invoice.id_rate=35 then(ar_payment_details.amount)   else 0 end) as total");	
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and  ar_invoice.id_rate in (select ar_invoice.id_rate from ar_invoice where ar_invoice.id_rate=30 or ar_invoice.id_rate=34  or ar_invoice.id_rate=31 or ar_invoice.id_rate=35 ) and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
	...
	$payed=$this->get_status("payed");
	$closed=$this->get_status("closed");
		$this->db->select("sum( case when ar_invoice.id_rate=30 or ar_invoice.id_rate=34 or ar_invoice.id_rate=31 or ar_invoice.id_rate=35 then (ar_payment_details.amount)  else 0 end) as total");	
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate in (select ar_invoice.id_rate from ar_invoice where ar_invoice.id_rate=30 or ar_invoice.id_rate=34  or  ar_invoice.id_rate=31 or ar_invoice.id_rate=35) and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
		$this->db->where($filter);		
		$query = $this->db->get("ar_payment_details  join ar_invoice on ar_payment_details.id_invoice=ar_invoice.id join sis_siswa on ar_invoice.id_student=sis_siswa.id join dm_kelas on sis_siswa.dm_kelas_id = dm_kelas.id join ar_payment on ar_payment_details.id_payment=ar_payment.id join ar_rate on ar_invoice.id_rate=ar_rate.id");
		return $query;
    ...
    $payed=$this->get_status("payed");
	$closed=$this->get_status("closed");
			$this->db->select("sum( case when  ar_invoice.id_rate=30 or ar_invoice.id_rate=34 then (ar_payment_details.amount)  else 0 end) as total");	
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate in (select ar_invoice.id_rate from ar_invoice where ar_invoice.id_rate=30 or ar_invoice.id_rate=34) and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
		$this->db->where($filter);		
		$query = $this->db->get("ar_payment_details  join ar_invoice on ar_payment_details.id_invoice=ar_invoice.id join sis_siswa on ar_invoice.id_student=sis_siswa.id join dm_kelas on sis_siswa.dm_kelas_id = dm_kelas.id join ar_payment on ar_payment_details.id_payment=ar_payment.id join ar_rate on ar_invoice.id_rate=ar_rate.id");
		return $query;
	}
	...
	$payed=$this->get_status("payed");
	$closed=$this->get_status("closed");  
			$this->db->select("sum( case when ar_invoice.id_rate=31 or ar_invoice.id_rate=35 then (ar_payment_details.amount)  else 0 end) as total");	
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate in (select ar_invoice.id_rate from ar_invoice where ar_invoice.id_rate=31 or ar_invoice.id_rate=35) and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
		$this->db->where($filter);		
		$query = $this->db->get("ar_payment_details  join ar_invoice on ar_payment_details.id_invoice=ar_invoice.id join sis_siswa on ar_invoice.id_student=sis_siswa.id join dm_kelas on sis_siswa.dm_kelas_id = dm_kelas.id join ar_payment on ar_payment_details.id_payment=ar_payment.id join ar_rate on ar_invoice.id_rate=ar_rate.id");
		return $query;
	}
