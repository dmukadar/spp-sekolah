...
*/ 	
	...
	$closed=$this->get_status("closed");
		...
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate=26 or ar_invoice.id_rate=27 or ar_invoice.id_rate=28 or ar_invoice.id_rate=29 or  ar_invoice.id_rate=39 and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
	...
	$closed=$this->get_status("closed");
		...
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate=26 or ar_invoice.id_rate=27 or ar_invoice.id_rate=28 or ar_invoice.id_rate=29 and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
		$this->db->where($filter);		
		$query = $this->db->get("ar_payment_details  join ar_invoice on ar_payment_details.id_invoice=ar_invoice.id join sis_siswa on ar_invoice.id_student=sis_siswa.id join dm_kelas on sis_siswa.dm_kelas_id = dm_kelas.id join ar_payment on ar_payment_details.id_payment=ar_payment.id join ar_rate on ar_invoice.id_rate=ar_rate.id");
		return $query;
	}
	...
	$closed=$this->get_status("closed");  
		...
		$this->db->where("sis_siswa.id = ar_invoice.id_student AND sis_siswa.dm_kelas_id = dm_kelas.id  AND sis_siswa.dm_jenjang_id = dm_kelas.dm_jenjang_id AND ar_invoice.id_rate=ar_rate.id and ar_payment.id=ar_payment_details.id_payment  and ar_payment_details.id_invoice=ar_invoice.id and ar_invoice.id_rate=39 and ar_payment_details.id_invoice=ar_invoice.id  and ar_payment_details.status IN (SELECT ar_payment_details.status FROM ar_rate WHERE ar_payment_details.status='".$payed."' or ar_payment_details.status='".$closed."')");	
		$this->db->where($filter);		
		$query = $this->db->get("ar_payment_details  join ar_invoice on ar_payment_details.id_invoice=ar_invoice.id join sis_siswa on ar_invoice.id_student=sis_siswa.id join dm_kelas on sis_siswa.dm_kelas_id = dm_kelas.id join ar_payment on ar_payment_details.id_payment=ar_payment.id join ar_rate on ar_invoice.id_rate=ar_rate.id");
		return $query;
	}
